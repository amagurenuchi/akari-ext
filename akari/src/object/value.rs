...
impl Value {
    ...
    /// let path = std::env::temp_dir().join("akari_write_test.json");
    /// object!({
    ///    key: "value",
    ///    number: 42,
    ///    list: [1, 2, 3],
    /// }).into_jsonf(path.to_str().unwrap()).expect("Failed to write JSON file");
    /// # }
    ///
    ...
    ///
    /// let path = std::env::temp_dir().join("akari_write_test.json");
    /// Value::Dict(dict).into_jsonf(path.to_str().unwrap()).expect("Failed to write JSON file");
    /// # }
    /// # }
    /// ```
    #[cfg(not(feature = "no_std"))]
    pub fn into_jsonf(&self, file_path: &str) -> Result<(), String> {
        let mut file =
            File::create(file_path).map_err(|e| format!("Failed to write JSON file: {}", e))?;
    ...
    /// let path = std::env::temp_dir().join("akari_test_read.json");
    /// fs::write(&path, r#"{"key": "value", "number": 42, "list": [1, 2, 3]}"#).unwrap();
    ///
    /// # #[cfg(feature = "object_macro")]
    /// # {
    /// use akari::object;
    /// // Read the JSON file and parse it into an Value
    /// let obj = Value::from_jsonf(path.to_str().unwrap()).expect("Failed to parse JSON file");
...
impl Into<char> for Value {
    fn into(self) -> char {
        self.string().chars().next().unwrap_or('\0')
    }
}
