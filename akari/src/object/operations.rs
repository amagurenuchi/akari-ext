...
impl Value {
    ...
    pub fn pow(&self, rhs: &Value) -> Value {
        match (self, rhs) {
            // Numerical ** Numerical
            (Value::Numerical(a), Value::Numerical(b)) => Value::Numerical(a.powf2(*b)),

            // Boolean conversions
            (Value::Boolean(a), Value::Boolean(b)) => {
                Value::Numerical((*a as i64 as f64).powf2(*b as i64 as f64))
            }
            (Value::Numerical(a), Value::Boolean(b)) => Value::Numerical(a.powf2(*b as i64 as f64)),
            (Value::Boolean(a), Value::Numerical(b)) => {
    ...
    pub fn try_pow(&self, rhs: &Value) -> Result<Value, ValueError> {
        match (self, rhs) {
            // Numerical ** Numerical
            (Value::Numerical(a), Value::Numerical(b)) => Ok(Value::Numerical(a.powf2(*b))),
    ...
    pub fn try_get_index(&self, index: &Value) -> Result<Value, ValueError> {
        match self {
            ...
            Value::Dict(dict) => {
                if let Value::Str(key) = index {
                    dict.get(key).cloned().ok_or(ValueError::KeyNotFoundError)
                } else {
                    Err(ValueError::TypeError)
                }
            }
...
mod tests {
    ...
    fn test_indexing() {
        ...
        assert_eq!(
            list.get_index(&Value::Numerical(1.0)),
            Value::Numerical(20.0)
        );
        assert_eq!(
            list.get_index(&Value::Numerical(-1.0)),
        ...
        assert_eq!(
            string.get_index(&Value::Numerical(1.0)),
        ...
        assert_eq!(
            dict_val.get_index(&Value::Str("key".to_string())),
            Value::Numerical(42.0)
        );
    }
    ...
    fn test_append() {
        ...
        let tuple = Value::List(vec![Value::Str("key2".to_string()), Value::Numerical(20.0)]);
