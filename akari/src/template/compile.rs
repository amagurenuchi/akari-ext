...
impl TemplateCompiler {
                    ...
                    if let Some(Token::Identifier(name)) = self.tokens.get(self.pos) {
                        ...
                        if let Some(block_tokens) = self.blocks.get(&block_name) {
                            ...
                            let mut block_compiler =
                                TemplateCompiler::new(block_tokens.clone(), self.data.clone());
                            ...
                            self.output
                                .push_str(&format!("<!-- Block '{}' not defined -->", block_name));
                        }
                        self.pos += 1;
                    } else {
    ...
    fn handle_variable_access(
            ...
            None => {
                if output_to_stream {
                    self.output.push_str(&format!(
                        "<!-- There should be a value '{}' but not found -->",
                        var_name
                    ));
                }
        ...
        while matches!(
            self.tokens.get(access_chain_pos),
            ...
            match &value {
                ...
                Obj::Dict(dict) => {
                    ...
                    if let Some(val) = dict.get(&key) {
                        ...
                        self.output
                            .push_str(&format!("<!-- Key '{}' not found in dictionary -->", key));
                        return Ok(Some(Obj::None));
                    } else {
            ...
            if let Some(Token::Identifier(prop_name)) = self.tokens.get(access_chain_pos).cloned() {
                ...
                match &value {
                    ...
                    _ => {
                        if output_to_stream {
                            self.output.push_str(&format!(
                                "<!-- Type '{}' does not support properties/methods -->",
                                value.type_of()
                            ));
                            return Ok(Some(Obj::None));
                        } else {
        ...
        if matches!(self.tokens.get(self.pos), Some(Token::Assignment)) {
            ...
            if self.pos == self.tokens.len()
                || !matches!(
                    self.tokens.get(access_chain_pos - 1),
                    Some(Token::RightSquareBracket) | Some(Token::Dot)
                )
            {
                ...
                return Err(
                    "Assignment to indexed/property values not fully implemented".to_string(),
                );
            }
    ...
    fn handle_deletion(&mut self) -> Result<(), String> {
        ...
        if let Some(Token::Identifier(name)) = self.tokens.get(self.pos) {
            ...
            if self.pos >= self.tokens.len()
                || !matches!(self.tokens.get(self.pos), Some(Token::LeftSquareBracket))
            {
            ...
            if let Some(collection) = self.data.get_mut(&var_name) {
                ...
                self.output
                    .push_str(&format!("<!-- Variable '{}' not found -->", var_name));
            }
    ...
    fn handle_for_loop(&mut self) -> Result<(), String> {
        ...
        if let Some(Token::Identifier(loop_var)) = self.tokens.get(self.pos).cloned() {
            ...
            match collection {
                Obj::List(items) => {
                    ...
                    for item in items_vec {
                        ...
                        let mut body_compiler =
                            TemplateCompiler::new(loop_body.clone(), self.data.clone());
                ...
                _ => {
                    self.output.push_str(&format!(
                        "<!-- For loop requires a list, dictionary, or number, got {} -->",
                        collection.type_of()
                    ));
                }
            }
    ...
    fn handle_while_loop(&mut self) -> Result<(), String> {
        ...
        while iteration < MAX_ITERATIONS {
            ...
            // Execute the loop body
            let mut body_compiler = TemplateCompiler::new(loop_body.clone(), self.data.clone());
            match body_compiler.generate_output() {
                Ok(body_output) => {
                    self.output.push_str(&body_output);
                    self.data = body_compiler.data;
                ...
                Err(e) => {
                    self.output
                        .push_str(&format!("<!-- Error in while loop: {} -->", e));
        ...
        match &self.tokens[self.pos] {
            ...
            Token::OutputKeyword => {
                ...
                match self.evaluate_expression() {
                    ...
                    Err(e) => self
                        .output
                        .push_str(&format!("<!-- Error evaluating expression: {} -->", e)),
                }
            }
            Token::LetKeyword => {
                if let Err(e) = self.handle_assignment() {
                    self.output
                        .push_str(&format!("<!-- Error in assignment: {} -->", e));
                }
            }
            Token::IfKeyword => {
                if let Err(e) = self.handle_if_statement() {
                    self.output
                        .push_str(&format!("<!-- Error in if statement: {} -->", e));
                }
            }
            Token::ForKeyword => {
                if let Err(e) = self.handle_for_loop() {
                    self.output
                        .push_str(&format!("<!-- Error in for loop: {} -->", e));
                }
            }
            Token::WhileKeyword => {
    ...
    fn evaluate_condition(&mut self) -> Result<bool, String> {
        match self.evaluate_expression() {
            Ok(expr_value) => match expr_value {
                Obj::Boolean(b) => Ok(b),
            ...
            Err(e) => {
                ...
                self.output
                    .push_str(&format!("<!-- Error in condition: {} -->", e));
                Ok(false)
            }
        }
    }
